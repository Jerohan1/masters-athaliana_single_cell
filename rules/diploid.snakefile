...
rule filter_star_reads_not_overlapping_variants:
    input:
        bam='../../aligned_data/diploid/{cond}.{comp}.sorted.bam',
        vcf='../../annotations/diploid/vcf/{comp}.snvs.vcf',
    output:
    ...
    conda:
        'env_yml/pysam.yml'
    shell: #uses -2 instead of -1 of vcf -> bed conversion as vcf; 1-based, bed; 0-based. -2 is for robustness padding. Adjusted chromosome name from vcf file to Chr4 as that is the denotion in the bam.
        '''
        awk -v OFS='\t' \
          '$0 !~ /^#/ {{print $1, $2 - 2, $2 + length($4), $3}}' \
          {input.vcf} > temp.snvs.bed
          
        awk 'BEGIN{{OFS="\t"}} {{$1="Chr4"; print}}' temp.snvs.bed > {output.bam}.snvs.bed
...
def get_gene_count_input(wc):
    ...
    return {
        ...
        'gtf_fn': ancient(config['genome_gff'][parent1]),
